...
class RetrievalService:
    ...
    def set_notebook_scope(self, notebook_id: str) -> Token[str | None]:
        ...
        return self._notebook_id.set(notebook_id)

    def reset_notebook_scope(self, token: Token[str | None]) -> None:
        """Reset the notebook scope after a request finishes."""
        self._notebook_id.reset(token)

    def _require_notebook_id(self) -> str:
        notebook_id = self._notebook_id.get()
        if not notebook_id:
            raise ValueError("RAG retrieval requires a notebook_id scope.")
        return notebook_id
    ...
    def _retrieve_multi_document(
        ...
        notebook_id = self._require_notebook_id()
        all_documents = self.doc_repo.list_documents(notebook_id)
